<template>
  ...
  const userId = route.params.uid // 🔒 Replace with dynamic value if needed
  ...
  for (let i = count - 1; i >= 0; i--) {
    ...
    const monthKey = format(pastDate, 'MMM yyyy') // 'May 2024', 'Apr 2024'...
  ...
  onMounted(async () => {
    ...
    // Utility to get previous months as 'MMM yyyy' labels
  ...
  scales: {
    x: {
      ...
      grid: {
        color: '#444' // soft grey grid lines
      }
    },
    y: {
      ticks: {
  ...
  canvas {
  background-color: #1e1e1e; /* deep grey */
  border-radius: 8px;
}
.add{
